...
| Backend | **FastAPI** (Python 3.11) | venv at `backend/venv`; all routes under `/api` |
| Frontend | **React + Vite + Tailwind** | dev server `localhost:5173` |
...
Supabase keys live in `backend/.env` (gitignored): `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` (secret — bypasses RLS), `SUPABASE_ANON_KEY` (publishable). The secret was rotated after accidental exposure.
...
**Health check:** `curl http://127.0.0.1:8000/api/countries` → 39 countries; `localhost:5173` → app loads.
...
6. **`api.js` has five HTTP methods:** `get`, `post`, `patch`, `put`, `delete`. The `put` method was missing until an earlier session and caused silent failures on PUT calls — always use `api.put(...)` for upserts, never fall back to `api.post(...)`.
7. **`assigned_counselor` and `created_by` on students/candidates** FK to `auth.users`, which is **EMPTY** (we run solo via service key). These fields are **deliberately omitted** from all create/update schemas and forms. Do NOT send them. Wire when auth is added.
...
11. **`service_fees` is finance-RLS-gated.** Uses `can_view_accounting()` for select/insert/update, matching the accounting tables. Once auth is wired, only owner/manager/accountant can see or edit fee records.
...
Countries, Institutes, Programs, AdmissionTemplates, PlacementTemplates, Industries, Employers, Jobs, DestinationExplorer, Students, Candidates, Inquiries, Applications, JobApplications, ReferralPartners, ServiceFees.
...
**Data seeded:** 39 countries (Japan = id 1); 16 SSW industry fields; JLPT/JFT/SSW qual types; one real institute (Yamaguchi University); one admission template (Japan Master's Research); one placement template (Japan Nursing Care SSW, country 1 / industry 1); one referral partner (Sakura Japanese Language Center, fixed 15000 BDT).

**Key milestones reached:** full CRUD all data pages; cascading selector both tracks; interactive roadmap progress tracking (students and candidates); Kanban pipelines (8-stage education, 7-stage employment); inquiry conversion (→student and →candidate, convert-once guard); referral partner + service fee tracking; forgiving client-side search.

---

## 7. Remaining Work (in order)

1. **Deferred checklists** (NEXT) — `application_checklist` and `job_application_checklist` tables already exist in DB. Need: seed items from `admission_requirements` on application create; UI to tick off per application in the Kanban drawer; mirror for `job_application_checklist`.
2. **Tasks UI, Accounting UI** (optionally wire `service_fees` into `transactions`); **Dashboards.**
3. **Authentication + RBAC** — backend uses service key (bypasses RLS entirely); `profiles` table is empty; `assigned_to`/`created_by`/`assigned_counselor` omitted everywhere. Wire Supabase Auth + JWTs. **Critical before any real staff log in.**
4. **Backend search endpoints** — current client-side search is fine for hundreds of records; add `?q=` to students/candidates GET if data grows large.
...
frontend/src/
  App.jsx                           — all routes
  lib/api.js                        — fetch wrapper; get/post/patch/put/delete
...
supabase/migrations/  — ~27 timestamped .sql files; push with `supabase db push`
```

---

*Snapshot as of June 25, 2026. Regenerate at the next milestone. Keep HANDOFF.md in sync.*

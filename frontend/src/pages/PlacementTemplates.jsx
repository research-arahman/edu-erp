...
export default function PlacementTemplates() {
  ...
  function loadTemplates() {
    return api.get('/placement-templates').then(setTemplates);
  }

  useEffect(() => {
    Promise.all([
      api.get('/placement-templates'),
      api.get('/countries'),
      api.get('/industries'),
    ])
      .then(([tmps, cnts, inds]) => {
  ...
  function openEdit(tmpl) {
    setForm({
      ...
      industry_field_id: tmpl.industry_field_id?.toString() ?? '',
      name:              tmpl.name ?? '',
    ...
    api.get(`/placement-templates/${tmpl.id}/steps`)
      .then(setSteps)
      .catch(() => setSteps([]))
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.country_id || !form.industry_field_id) {
      setFormError('Country and industry field are required.');
      return;
    }
    setSaving(true);
    setFormError(null);
    try {
      const payload = {
        ...
        industry_field_id: Number(form.industry_field_id),
      };
      if (form.name.trim())        payload.name        = form.name.trim();
      if (form.description.trim()) payload.description = form.description.trim();

      if (panel === 'add') {
        await api.post('/placement-templates', payload);
      } else {
        await api.patch(`/placement-templates/${selected.id}`, payload);
      }
      await loadTemplates();
      closePanel();
    } catch (err) {
  ...
  async function handleDelete(e, tmpl) {
    e.stopPropagation();
    const industryName = industryMap[tmpl.industry_field_id]?.name ?? 'Industry';
    const label = tmpl.name || `${countryMap[tmpl.country_id] ?? 'Template'} — ${industryName}`;
    if (!window.confirm(`Delete "${label}"? This cannot be undone.`)) return;
    try {
      await api.delete(`/placement-templates/${tmpl.id}`);
  ...
  async function handleAddStep(e) {
    ...
    try {
      ...
      const newStep = await api.post(`/placement-templates/${selected.id}/steps`, payload);
  ...
  async function handleDeleteStep(step) {
    ...
    try {
      await api.delete(`/placement-steps/${step.id}`);
  ...
  return (
    <>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Placement Templates</h2>
          {!loading && !error && (
        ...
        <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                {['Country', 'Industry Field', 'Name', 'Steps', ''].map((col) => (
                  <th
                    key={col}
              ...
              {templates.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-5 py-16 text-center text-sm text-gray-400">
                    No placement templates yet — add one above.
                  </td>
                </tr>
              ) : (
                templates.map((tmpl) => {
                  const ind = industryMap[tmpl.industry_field_id];
                  return (
                    <tr
                      key={tmpl.id}
                      ...
                      <td className="px-5 py-3 text-gray-800">
                        {ind ? (
                          <>
                            {ind.name}
                            {ind.is_ssw && (
                              <span className="ml-1.5 inline-block rounded bg-amber-50 px-1.5 py-0.5 text-xs font-semibold text-amber-700">
                                SSW
                              </span>
                            )}
                          </>
          ...
          <div className="fixed inset-y-0 right-0 z-50 flex w-[540px] flex-col bg-white shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-y-auto">
              <div className="flex-1 space-y-4 px-6 py-5">
                ...
                <Field label="Industry Field" required>
                  <select
                    className={INPUT}
                ...
                <Field label="Template Name">
                  <input
                    ...
                    onChange={handleChange}
                    disabled={saving}
                    placeholder="e.g. Japan Nursing Care (SSW)"
                  />
                </Field>
                ...
                <Field label="Description">
                  <textarea
                    ...
                    disabled={saving}
                    rows={3}
                    placeholder="Optional notes about this placement process…"
                  />
                </Field>
                  ...
                  <div className="mt-2 rounded-lg border border-gray-200 bg-gray-50 p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        onChange={handleStepChange}
                        placeholder="Timeframe (e.g. before applying)"
                        disabled={addingStep}
                      />
                      <textarea
                        className={INPUT + ' text-xs'}
